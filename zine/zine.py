...
def parse_args():
    ...
    parser.add_argument("-c", "--count", metavar='N', help="number of photos in the zine",
                        type=int)
    parser.add_argument("-o", "--output", help="destination file", default="output.pdf")
...
def main():
    ...
    with open(base_path + '/static/template.html.jinja2') as file_:
        template = Template(file_.read())
        source = template.render(photos=mangled)
        with open(base_path + '/static/style.css') as style_:
